...
class SimulatedAnnealing:
    ...
    """
    def __init__(self, initial_solution, solution_evaluator, get_neighbor, cooling_schedule='linear', step_max=1000,
                 t_min=0, t_max=100, bounds=[], alpha=None, damping=1):

        # checks
        assert cooling_schedule in ['linear', 'exponential', 'logarithmic', 'quadratic'], \
            'cooling_schedule must be either "linear", "exponential", "logarithmic", or "quadratic"'
        ...
        # initialise neighbor function
        self.get_neighbor = get_neighbor

        # initialise cost func from simulation
        self.cost_func = solution_evaluator
